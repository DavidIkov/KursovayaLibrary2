...
public:
    ...
    //be aware that this constructor will not call move contructors for T, it will use copy constructors
    ...
    constexpr Matrix<SizeX, SizeY, T>& operator=(const Matrix<SizeX, SizeY, T>& matToCopy) noexcept {
        this->~Matrix();
        new(this) Matrix(matToCopy);
        return *this;
    }
    constexpr Matrix<SizeX, SizeY, T>& operator=(Matrix<SizeX, SizeY, T>&& matToCopy) noexcept {
        this->~Matrix();
    ...
    constexpr T GetDeterminant() const noexcept {
        ...
        if constexpr (SizeX == 1) {
            ...
            for (size_t ai = 0; ai < SizeX; ai++) {
                ...
                det += Nums[ai * SizeY] * multiplier * Matrix<SizeX - 1, SizeX - 1, T>(&tempNums[0]).GetDeterminant();
    ...
    constexpr Matrix<SizeX, SizeY, T> GetInversedMatrix(const T& det) const noexcept {
        ...
        for (size_t x = 0; x < SizeX; x++) {
            for (size_t y = 0; y < SizeY; y++) {
                ...
                retMat.Nums[y + x * SizeY] = multipler * Matrix<SizeX - 1, SizeY - 1, T>(tempNums).GetDeterminant() / det;
                multipler = -multipler;
            }
        }
    ...
    //used to normalize whole matrix, since after some rotations matrix can get a little broken becouse of Ting point precision error
    ...
    constexpr T GetLocalCordForAxisC(const Vector<SizeY, T>& vec) const noexcept {
    ...
    //works only for Cartesian coordinates system
    template<size_t axisInd>
    constexpr Vector<SizeX, T> GetLocalCordsForAxisC(const Vector<SizeY, T>& vec) const noexcept {
    ...
    constexpr Vector<SizeX, T> GetLocalCordsForAxisU(const Vector<SizeY, T>& vec) const noexcept {
    ...
    Vector<SizeY, T> RotateVectorByTwoVectorsU(const Vector<SizeY, T>& vec, const float angle) const noexcept {
        return RotateVectorByTwoVectorsU<axisForX, axisForY>(vec, angle, this->GetInversedMatrix(this->GetDeterminant()));
    }
    template<size_t axisForX, size_t axisForY>
    Vector<SizeY, T> RotateVectorByTwoVectorsU(const Vector<SizeY, T>& vec, const float angle, const T det) const noexcept {
        return RotateVectorByTwoVectorsU<axisForX, axisForY>(vec, angle, this->GetInversedMatrix(det));
    }
    template<size_t axisForX, size_t axisForY>
    Vector<SizeY, T> RotateVectorByTwoVectorsU(const Vector<SizeY, T>& vec, const float angle, const Matrix<SizeX,SizeY,T>& invMat) const noexcept {
    ...
    Vector<SizeY, T> RotateVectorC(const Vector<SizeY, T>& vec, const T angle) const noexcept {
        ...
        Vector<SizeY, T> xv(&Nums[SizeY * axisX]); Vector<SizeY, T> yv(&Nums[SizeY * axisY]);

        return (xv * cosf(angleToX) + yv * sinf(angleToX)) * vecLen;
    }

    template<size_t axis1, size_t axis2, size_t axis3>
    Matrix<SizeX, SizeY, T> RotateIn3DByAnglesC(const Vector3F& rot) const noexcept {
    ...
    template<size_t axis1, size_t axis2, size_t axis3>
    Matrix<SizeX, SizeY, T> RotateIn3DByAnglesC(const float xr, const float yr, const float zr) const noexcept {
...
#define rotVecMacr(i){\
        ...
        Vector<3, T> xv(&retMat.Nums[SizeY * xvi]); Vector<3, T> yv(&retMat.Nums[SizeY * yvi]);\
        Vector<3, T> nxv = xv * cosf(rots[order[i]]) + yv * sinf(rots[order[i]]);\
        Vector<3, T> nyv = xv * -sinf(rots[order[i]]) + yv * cosf(rots[order[i]]);\
...
#undef rotVecMacr
    ...
    constexpr Matrix<SizeX, SizeY, T> operator+(const T& num) const noexcept { Matrix<SizeX, SizeY, T> retMat; for (size_t i = 0; i < SizeX * SizeY; i++) retMat.Nums[i] = Nums[i] + num; return retMat; }
    constexpr Matrix<SizeX, SizeY, T> operator-(const T& num) const noexcept { Matrix<SizeX, SizeY, T> retMat; for (size_t i = 0; i < SizeX * SizeY; i++) retMat.Nums[i] = Nums[i] - num; return retMat; }
    constexpr Matrix<SizeX, SizeY, T> operator*(const T& num) const noexcept { Matrix<SizeX, SizeY, T> retMat; for (size_t i = 0; i < SizeX * SizeY; i++) retMat.Nums[i] = Nums[i] * num; return retMat; }
    constexpr Matrix<SizeX, SizeY, T> operator/(const T& num) const noexcept { Matrix<SizeX, SizeY, T> retMat; for (size_t i = 0; i < SizeX * SizeY; i++) retMat.Nums[i] = Nums[i] / num; return retMat; }

    template<size_t SizeX2>
    constexpr Matrix<SizeX2, SizeY, T> operator*(const Matrix<SizeX2, SizeX, T>& mat) const noexcept {
    ...
    constexpr Vector<SizeY, T> operator*(const Vector<SizeY, T>& vec) const noexcept {
