...
public:
    ...
    constexpr Vector<AxesAmount, T>& operator=(const Vector<AxesAmount, T>& vecToCopy) noexcept {
        this->~Vector();
        new(this) Vector(vecToCopy);
        return *this;
    }
    constexpr Vector<AxesAmount, T>& operator=(Vector<AxesAmount, T>&& vecToCopy) noexcept {
        this->~Vector();
    ...
    constexpr T& operator[](const size_t ind) noexcept {
        return Axes[ind];
    }
    ...
    constexpr T LengthSQ() const noexcept {
        T sum = T(); for (size_t i = 0; i < AxesAmount; i++) sum += Axes[i] * Axes[i];
        return sum;
    }
        ...
        const Vector<AxesAmount, T>& p2, const Vector<AxesAmount, T>& v2) noexcept
    {
        ...
        T v1l = v1.Length(); T v2l = v2.Length();

        //variables for system like
        // {ax+by+c=0
        // {dx+ey+f=0
        T a = v1.DotFL(v2); T b = -v2l*v2l; T c = diff.DotFL(v2);
        T d = v1l*v1l; T e = -v2.DotFL(v1); T f = diff.DotFL(v1);

        return Vector<2, T>((b * f - e * c) / (a * e - d * b), (d * c - f * a) / (a * e - d * b));
    };
        ...
        const Vector<AxesAmount, T>& p2, const Vector<AxesAmount, T>& v2) noexcept
    {
        ...
        Vector<AxesAmount, T> mp1 = p1 + dists[0]* v1; Vector<AxesAmount, T> mp2 = p2 + dists[1]* v2; 

        return (mp1 - mp2).Length();
    };

    //cross product of full length, means that it wasnt deivided by length of both vectors
    constexpr Vector<AxesAmount, T> CrossFL(const Vector<AxesAmount, T>& vec) const noexcept {
        static_assert(AxesAmount == 3, "Cross is defined only for 3-axes vectors!");
        return Vector<AxesAmount, T>(Axes[1] * vec.Axes[2] - Axes[2] * vec.Axes[1], Axes[2] * vec.Axes[0] - Axes[0] * vec.Axes[2], Axes[0] * vec.Axes[1] - Axes[1] * vec.Axes[0]);
    }
    Vector<AxesAmount, T> Cross(const Vector<AxesAmount, T>& vec) const noexcept {
        return CrossFL(vec) / Length() / vec.Length();
    }


    constexpr Vector<AxesAmount, T> operator+(const T& num) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] + num; return retVec; }
    constexpr Vector<AxesAmount, T> operator-(const T& num) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] - num; return retVec; }
    constexpr Vector<AxesAmount, T> operator*(const T& num) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] * num; return retVec; }
    constexpr Vector<AxesAmount, T> operator/(const T& num) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] / num; return retVec; }

    constexpr Vector<AxesAmount, T> operator+(const Vector<AxesAmount, T>& vec) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] + vec.Axes[i]; return retVec; }
    constexpr Vector<AxesAmount, T> operator-(const Vector<AxesAmount, T>& vec) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] - vec.Axes[i]; return retVec; }
    constexpr Vector<AxesAmount, T> operator*(const Vector<AxesAmount, T>& vec) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] * vec.Axes[i]; return retVec; }
    constexpr Vector<AxesAmount, T> operator/(const Vector<AxesAmount, T>& vec) const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = Axes[i] / vec.Axes[i]; return retVec; }

    constexpr Vector<AxesAmount, T> operator-() const noexcept { Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec.Axes[i] = -Axes[i]; return retVec; }

    constexpr Vector<AxesAmount, T>& operator+=(const T& num) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] += num; return *this; }
    constexpr Vector<AxesAmount, T>& operator-=(const T& num) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] -= num; return *this; }
    constexpr Vector<AxesAmount, T>& operator*=(const T& num) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] *= num; return *this; }
    constexpr Vector<AxesAmount, T>& operator/=(const T& num) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] /= num; return *this; }

    constexpr Vector<AxesAmount, T>& operator+=(const Vector<AxesAmount, T>& vec) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] += vec.Axes[i]; return *this; }
    constexpr Vector<AxesAmount, T>& operator-=(const Vector<AxesAmount, T>& vec) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] -= vec.Axes[i]; return *this; }
    constexpr Vector<AxesAmount, T>& operator*=(const Vector<AxesAmount, T>& vec) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] *= vec.Axes[i]; return *this; }
    constexpr Vector<AxesAmount, T>& operator/=(const Vector<AxesAmount, T>& vec) noexcept { for (size_t i = 0; i < AxesAmount; i++) Axes[i] /= vec.Axes[i]; return *this; }

    template<typename T2>
    constexpr operator Vector<AxesAmount, T2>() const noexcept { Vector<AxesAmount, T2> vec; for (size_t i = 0; i < AxesAmount; i++) vec.Axes[i] = (T2)Axes[i]; return vec; }
    
    constexpr bool operator==(const Vector<AxesAmount, T>& vec) noexcept { for (size_t i = 0; i < AxesAmount; i++) if (Axes[i] != vec.Axes[i]) return false; return true; }
    constexpr bool operator!=(const Vector<AxesAmount, T>& vec) noexcept { for (size_t i = 0; i < AxesAmount; i++) if (Axes[i] != vec.Axes[i]) return true; return false; }
};


template<size_t AxesAmount, typename T>
constexpr Vector<AxesAmount, T> operator+(const T& num, const Vector<AxesAmount, T>& vec) noexcept {
    Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec[i] = num + vec[i]; return retVec;
}
template<size_t AxesAmount, typename T>
constexpr Vector<AxesAmount, T> operator-(const T& num, const Vector<AxesAmount, T>& vec) noexcept {
    Vector<AxesAmount, T> retVec; for (size_t i = 0; i < AxesAmount; i++) retVec[i] = num - vec[i]; return retVec;
